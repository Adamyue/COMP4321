...
\begin{lstlisting}[language=Python, caption=Test Case 1.2 Stop Word and Stemming Test Program]
...
\begin{center}
    \includegraphics[width=0.90\linewidth]{../screenshots/test_1_2.png}

    \textbf{Figure:} Test Case 1.2 terminal output showing stop-word lookup, Porter stemming examples, and normalized indexed terms.
\end{center}
    ...
    \item \textbf{Test Case 1.4: Re-crawling Robustness}: The crawler compares the current HTTP modification/date value with the stored value and re-indexes a page only when the values differ.
\end{itemize}
...
\begin{center}
    \includegraphics[width=0.90\linewidth]{../screenshots/web_interface.png}

    \textbf{Figure:} Web interface test showing a query submitted through Flask and ranked results rendered with scores, titles, URLs, keywords, parent links, and child links.
\end{center}
...
\begin{itemize}
    \item \textbf{YUE Pu}: Indexer search engine retrieval function.
